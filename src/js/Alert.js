
// "use strict"

// var myInit = {
//             method : "GET",
//             headers: {
//               "Content-Type": "application/json"
//             },
//             mode: "cors",
//             cache: "default" };

// let myRequest = new Request("../alerts.json", myInit);

// fetch(myRequest)
//   .then(function(resp){
//       return resp.json();
//   })
//   .then(function(data){
//     console.log(data)
//   })

  // function showInfo(data){
  //   console.log(data);
  // }


// import data from "../alerts.json" assert {type:'json'}


// export default class alert {
//     constructor(filePath) {
//         this.filePath = filePath;
//     }


//     //This method will show the meaning of the parameter, in this case the key of the alert.
//     readAlert(keyName) {
      
//       // section where i append the data of the alert

//       // let div_main = document.getElementById("alert-section");
      
//       // section where i append the data of the alert
            
//             try {
                
//                 // Iterate over the data if it's an array
//                 if (Array.isArray(data)) {
//                     data.forEach(alert => {
//                         // let section = document.createElement('section');
//                         // let paragraph = document.createElement('p');
//                         // paragraph.textContent = alert[keyName];
//                         // section.appendChild(paragraph);
//                         // section.classList.add("alert-list");
//                         // section.style.backgroundColor = alert.color;
//                         // div_main.appendChild(section);
//                         console.log('Message:', alert.message);
//                         console.log('Background:', alert.background);
//                         console.log('Color:', alert.color);
//                         console.log('---------------------------');
//                         console.log(`Key´s "${keyName}":`, alert[keyName]);
//                         console.log(' ');
//                     });
//                 } 
//             } catch (parseError) {
//                 console.error('Error parsing JSON:', parseError);
//             }
//     }
// }

// // // Example usage:
// const filePath = data

// const jsonReader = new alert(filePath);
// jsonReader.readAlert('message'); 

// Specify the key name here













